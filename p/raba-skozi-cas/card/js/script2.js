...
$(document).ready(function() {
    ...
    $.getJSON('https://isci.parlameter.si/q/kriza', function(r) {
        raw_data = r['facet_counts']['facet_dates']['datetime_dt'];

        var dates = [];
        var occurences = [];

        var date_formatter = d3.time.format('%Y-%m-%dT%H:%M:%S.Z');

        for (piece in raw_data) {
            dates.push(date_formatter.parse(piece));
            occurences.push(raw_data[piece]);
            console.log(piece);
            console.log(date_formatter.parse(piece));
        }

        data = [{
            'label': 'Raba skozi čas',
            x: dates,
            y: occurences
        }, {
            'label': 'Raba skozi čas',
